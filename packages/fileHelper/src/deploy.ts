...
class ScpClient {
  ...
  async uploadDir(localPath: string, remotePath: string, exclude: string[] = [], sftp: any): Promise<void> {
    ...
    const uploadPromises = files.map(file => {
      if (exclude.includes(file)) return Promise.resolve();
...
async function getDiffFilePath(fallback: string): Promise<string> {
  try {
    ...
    const diffFilePath = path.join(process.cwd(), bacfg.reportDir, `${path.basename(bacfg.buildDir)}.zip`);
...
export async function deploy(options: Partial<DeployConfig> = {}): Promise<void> {
  ...
  try {
    ...
    const stats = await fs.stat(localPath);
    await client.upload(localPath, config.remotePath, config.exclude, stats.isFile() ? false : config.compress);
