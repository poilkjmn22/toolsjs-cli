...
class ScpClient {
    ...
    async uploadDir(localPath, remotePath, exclude = [], sftp) {
        ...
        const uploadPromises = files.map(file => {
            ...
            if (exclude.some(pattern => {
                return (0, minimatch_1.default)(localFilePath, pattern) ||
    ...
    async preDeployAutoBackup(remotePath) {
        try {
...
async function getDiffFilePath(fallback) {
    try {
        const bacfgCont = await (0, promises_1.readFile)(path.join(process.cwd(), "buildAnalyzer.config.json"), 'utf-8');
